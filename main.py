...
class LoginBrute:
    async def main():
        ...
        parser.add_argument(
            ...
            "--url",
            type=str,
            help="specify the url to the form",
            required=True,
        )
        parser.add_argument(
            "-p",
            "--parallel",
            help="submit each username with the parallel password in the file",
            action="store_true",
        )
        ...
        parser.add_argument(
            "-j",
            "--javascript",
            help="initiate the attack with headless browser",
            action="store_true",
        )
        ...
        # Retrieve login page technology, for example: wordpress...
        web_info = WebInfo.webInfo(url, args["Technologies"], is_javascript)
        web_parser = webParser.webParser()
        await web_parser.getsource(web_info.url, args["paramsList"], pass_user)
        ...
        if web_info.is_javascript:
            ...
            html_brute = htmlBrute.htmlBrute(
                web_info.url, web_parser, pass_user, parallel
            )
            html_brute.brute()
